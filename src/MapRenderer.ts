...
export class MapRenderer {
  public static async renderMapToBuffer(engine: CatanEngine): Promise<Buffer> {
    ...
    for (const hex of engine.map) {
      ...
      ctx.closePath();
      ctx.fillStyle = getColor(hex.resource); ctx.fill();
      ctx.strokeStyle = "#FFFFFF"; ctx.lineWidth = 2; ctx.stroke();
      if (hex.resource !== ResourceType.DESERT) {
        ctx.beginPath(); ctx.arc(offsetX + hex.x, offsetY + hex.y, 16, 0, Math.PI * 2);
    ...
    engine.edges.forEach((edge, id) => {
        ...
        if (ownerId) {
            ...
            ctx.beginPath(); ctx.moveTo(offsetX + n1.x, offsetY + n1.y); ctx.lineTo(offsetX + n2.x, offsetY + n2.y);
            ctx.strokeStyle = owner.color; ctx.lineWidth = 8; ctx.lineCap = "round"; ctx.stroke();
        }
    });
    engine.settlements.forEach((b, nodeId) => {
        const node = engine.nodes.get(nodeId)!;
        const owner = engine.players.find(p => p.id === b.playerId)!;
        ctx.fillStyle = owner.color; ctx.strokeStyle = "#000000"; ctx.lineWidth = 2;
        if (b.type === BuildingType.CITY) { ctx.beginPath(); ctx.rect(offsetX + node.x - 12, offsetY + node.y - 12, 24, 24); ctx.fill(); ctx.stroke(); }
        else { ctx.beginPath(); ctx.arc(offsetX + node.x, offsetY + node.y, 10, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); }
    });
  ...
  public static async renderInteractiveMap(engine: CatanEngine, validSpots: {id: string, label: string}[], isNode: boolean): Promise<Buffer> {
     ...
     for (const spot of validSpots) {
         let x, y;
         if (isNode) { const n = engine.nodes.get(spot.id)!; x = n.x; y = n.y; }
         else { const e = engine.edges.get(spot.id)!; const n1 = engine.nodes.get(e.node1Id)!, n2 = engine.nodes.get(e.node2Id)!; x = (n1.x + n2.x) / 2; y = (n1.y + n2.y) / 2; }
         ctx.beginPath(); ctx.arc(offsetX + x, offsetY + y, 12, 0, Math.PI * 2); ctx.fillStyle = "#FFFFFF"; ctx.fill(); ctx.strokeStyle = "#E91E63"; ctx.lineWidth = 2; ctx.stroke();
         ctx.fillStyle = "#000000"; ctx.font = "bold 12px Arial"; ctx.textAlign = "center"; ctx.textBaseline = "middle"; ctx.fillText(spot.label, offsetX + x, offsetY + y);
     }
     return canvas.toBuffer("image/png");
  }
}
