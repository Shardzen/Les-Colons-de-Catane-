...
export class MapRenderer {
  public static async renderMapToBuffer(engine: CatanEngine): Promise<Buffer> {
    ...
    engine.settlements.forEach((b, nodeId) => {
        ...
        if (b.type === BuildingType.CITY) {
            ctx.beginPath(); ctx.rect(offsetX + node.x - 12, offsetY + node.y - 12, 24, 24); ctx.fill(); ctx.stroke();
        } else {
            ctx.beginPath(); ctx.arc(offsetX + node.x, offsetY + node.y, 10, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
        }
    });
  ...
  public static async renderInteractiveMap(engine: CatanEngine, validSpots: {id: string, label: string}[], isNode: boolean): Promise<Buffer> {
     ...
     for (const spot of validSpots) {
         let x, y;
         if (isNode) {
             const n = engine.nodes.get(spot.id)!; x = n.x; y = n.y;
         } else {
         ...
         ctx.beginPath(); ctx.arc(offsetX + x, offsetY + y, 12, 0, Math.PI * 2);
